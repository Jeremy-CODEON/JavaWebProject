		...
		<header class="topbar" style="margin-top: 0px;">
			<nav class="navbar top-navbar navbar-expand-md navbar-light">
				<div class="navbar-header">
					<a class="navbar-brand" href="saler"> <!-- Logo icon --> <b>
					...
					<ul class="navbar-nav my-lg-0">
						...
						<c:choose>
							<c:when test="${sessionScope.saler!=NULL}">
								<li class="nav-item dropdown u-pro"><a href="salerEnter"
									class="nav-link dropdown-toggle waves-effect waves-dark profile-pic"
									data-toggle="dropdown" aria-haspopup="true"
									aria-expanded="false"><span class="hidden-md-down">欢迎销售(编号:${sessionScope.saler.id} 负责音乐类型:${sessionScope.saler.sale_category})！
											&nbsp;</span></a></li>
							</c:when>
							<c:otherwise>
							...
							<div class="card-body">
								...
								<div class="table-responsive m-t-20 no-wrap">
									<table class="table vm no-th-brd pro-of-month">
										...
										<tbody>
											<tr>
												<td id="cateName">${requestScope.cateInfo.category}</td>
												<td id="cateMusicNum">${requestScope.cateInfo.musicNum}</td>
												<td id="cateSaleNum">${requestScope.cateInfo.totalSaleNum}</td>
												<td id="cateSale">${requestScope.cateInfo.totalSale}</td>
											</tr>
										</tbody>
									</table>
								</div>
		...
		function buttonDelete(id) {
			$.ajax({
				...
				success : function(json) {
					alert(json.message);
					window.location.replace("salerEnter");
				},
				error : function() {
					alert("请求删除音乐出错！");
				}
